...
// Migración automática: agregar columna tipo_relacion si no existe
$checkColumn = $conn->query("SHOW COLUMNS FROM empleados_proveedor LIKE 'tipo_relacion'");
if ($checkColumn && $checkColumn->num_rows == 0) {
    $conn->query("ALTER TABLE empleados_proveedor ADD tipo_relacion VARCHAR(20) DEFAULT 'dependencia'");
}
require_once 'vendor/autoload.php'; // PhpSpreadsheet
use PhpOffice\PhpSpreadsheet\IOFactory;

// Solo permite acceso a proveedores
...
// Obtener proveedor_id de forma robusta (debe ir antes de cualquier procesamiento)
if (isset($_POST['proveedor_id']) && is_numeric($_POST['proveedor_id'])) {
    $proveedor_id = intval($_POST['proveedor_id']);
} elseif (isset($_SESSION['proveedor_id']) && is_numeric($_SESSION['proveedor_id'])) {
    // prefer canonical proveedor_id set at login
    $proveedor_id = intval($_SESSION['proveedor_id']);
} else {
    // legacy fallbacks
    $proveedor_id = $_SESSION['usuario_id'] ?? $_SESSION['id'] ?? 0;
}
        ...
        try {
            ...
            foreach ($sheet->getRowIterator() as $i => $row) {
                ...
                // Suponiendo columnas: nombre, apellido, cuil, puesto, tipo_relacion (opcional)
                    ...
                    $stmt = $conn->prepare("SELECT id FROM empleados_proveedor WHERE cuil = ? AND proveedor_id = ?");
                    $stmt->bind_param("si", $data[2], $proveedor_id);
                    $stmt->execute();
                    $stmt->store_result();
                    if ($stmt->num_rows == 0) {
                        $stmt->close();
                        $stmt = $conn->prepare("INSERT INTO empleados_proveedor (proveedor_id, nombre, apellido, cuil, puesto, tipo_relacion, fecha_alta) VALUES (?, ?, ?, ?, ?, ?, NOW())");
                        $stmt->bind_param("isssss", $proveedor_id, $data[0], $data[1], $data[2], $data[3], $tipo_relacion);
                        $stmt->execute();
                        $stmt->close();
                        $rowCount++;
                    } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['agregar_empleado_proveedor'])) {
    ...
    if ($nombre && $apellido && $cuil && $puesto) {
        ...
        $stmt = $conn->prepare("SELECT id FROM empleados_proveedor WHERE cuil = ?");
        ...
        if ($stmt->num_rows > 0) {
            ...
            $stmt->close();
            $stmt = $conn->prepare("INSERT INTO empleados_proveedor (proveedor_id, nombre, apellido, cuil, puesto, tipo_relacion, fecha_alta) VALUES (?, ?, ?, ?, ?, ?, NOW())");
            $stmt->bind_param("isssss", $proveedor_id, $nombre, $apellido, $cuil, $puesto, $tipo_relacion);
            if ($stmt->execute()) {
...
if (isset($_POST['eliminar_empleado']) && isset($_POST['empleado_id'])) {
    $stmt = $conn->prepare("DELETE FROM empleados_proveedor WHERE id = ? AND proveedor_id = ?");
    $stmt->bind_param("ii", $_POST['empleado_id'], $proveedor_id);
    if ($stmt->execute()) {
...
if (isset($_POST['editar_empleado']) && isset($_POST['empleado_id'])) {
    $stmt = $conn->prepare("SELECT id, nombre, apellido, cuil, puesto FROM empleados_proveedor WHERE id = ? AND proveedor_id = ?");
    $stmt->bind_param("ii", $_POST['empleado_id'], $proveedor_id);
    $stmt->execute();
...
if (isset($_POST['guardar_edicion']) && isset($_POST['empleado_id'])) {
    ...
    if ($nombre && $apellido && $cuil && $puesto) {
        ...
        $stmt = $conn->prepare("SELECT id FROM empleados_proveedor WHERE cuil = ? AND id != ?");
        $stmt->bind_param("si", $cuil, $_POST['empleado_id']);
        $stmt->execute();
        $stmt->store_result();
        if ($stmt->num_rows > 0) {
            ...
            $stmt->close();
            $stmt = $conn->prepare("UPDATE empleados_proveedor SET nombre = ?, apellido = ?, cuil = ?, puesto = ?, tipo_relacion = ? WHERE id = ? AND proveedor_id = ?");
            $stmt->bind_param("ssssssi", $nombre, $apellido, $cuil, $puesto, $tipo_relacion, $_POST['empleado_id'], $proveedor_id);
            if ($stmt->execute()) {
...
$empleados = [];
$stmt = null;
// When migrating from older data, some rows may have proveedor_id set to the users.id (usuario_id).
$legacy_uid = $_SESSION['usuario_id'] ?? $_SESSION['id'] ?? 0;
if ($legacy_uid && $legacy_uid != $proveedor_id) {
    $stmt = $conn->prepare("SELECT id, nombre, apellido, cuil, puesto, tipo_relacion, fecha_alta FROM empleados_proveedor WHERE proveedor_id = ? OR proveedor_id = ? ORDER BY fecha_alta DESC");
    $stmt->bind_param("ii", $proveedor_id, $legacy_uid);
} else {
    $stmt = $conn->prepare("SELECT id, nombre, apellido, cuil, puesto, tipo_relacion, fecha_alta FROM empleados_proveedor WHERE proveedor_id = ? ORDER BY fecha_alta DESC");
...
<head>
    ...
    <script>
        ...
        function limpiarFormulario() {
            window.location.href = 'alta_empleados_proveedor.php';
        }
    </script>
</head>
<body class="min-h-screen">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div class="card lg:col-span-2">
                ...
                <div class="p-6">
                    ...
                    ?>
                    <!-- Sticky filter bar for provider's employees -->
                    <div id="empprov-filter-bar" style="position:sticky; top:96px; z-index:998; display:flex; gap:0.5rem; align-items:center; margin-bottom:1rem; background: rgba(255,255,255,0.9); padding:8px 10px; border-radius:10px; width:100%;">
                        <input id="empprov-search" type="search" placeholder="Buscar por nombre, cuil o puesto" class="form-input" style="flex:1; min-width:320px;" />
                        <select id="empprov-filter" class="form-input" style="width:140px;">
                            <option value="">Puesto</option>
                            <?php foreach($puestos as $p): ?>
                                <option value="<?= htmlspecialchars($p) ?>"><?= htmlspecialchars($p) ?></option>
                            <?php endforeach; ?>
                        </select>
                        <select id="empprov-relacion" class="form-input" style="width:140px;">
                            <option value="">Relación</option>
                            <option value="dependencia">Dependencia</option>
                            <option value="monotributista">Monotributista</option>
                        </select>
                        <button type="button" id="empprov-clear" class="btn-secondary" style="padding:8px 10px; font-size:0.95rem;">Limpiar</button>
                    </div>
                    <div id="empprov-noresults" class="text-center py-4" style="display:none;">
                    ...
                    <?php if (count($empleados) > 0): ?>
                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-4 max-h-96 overflow-y-auto lg:col-span-2" id="empleados-prov-list">
    ...
    <script>
        ...
        // Filtrado cliente para empleados del proveedor (buscar por nombre, cuil, puesto, tipo_relacion)
        document.addEventListener('DOMContentLoaded', function(){
            var empCards = Array.from(document.querySelectorAll('#empleados-prov-list .empleado-card'));
            var empSearch = document.getElementById('empprov-search');
            var empFilter = document.getElementById('empprov-filter');
            var empRelacion = document.getElementById('empprov-relacion');
            var empClear = document.getElementById('empprov-clear');
            var empNoResults = document.getElementById('empprov-noresults');
            // emp-filter-bar is sticky and should remain visible; hide-on-scroll logic removed

            function applyEmpProvFilter(){
                ...
                var cards = Array.from(document.querySelectorAll('#empleados-prov-list .empleado-card'));
                ...
                if (empNoResults) {
                    var anyVisible = Array.from(document.querySelectorAll('#empleados-prov-list .empleado-card')).some(function(c){ return c.style.display !== 'none'; });
                    empNoResults.style.display = anyVisible ? 'none' : '';
                }
            }
            if (empSearch) empSearch.addEventListener('input', function(){ clearTimeout(empFilterTimeout); empFilterTimeout = setTimeout(applyEmpProvFilter, 200); });
            if (empFilter) empFilter.addEventListener('change', applyEmpProvFilter);
            if (empRelacion) empRelacion.addEventListener('change', applyEmpProvFilter);
            if (empClear) empClear.addEventListener('click', function(){ if (empSearch) empSearch.value=''; if (empFilter) empFilter.value=''; if (empRelacion) empRelacion.value=''; applyEmpProvFilter(); });
        });
    </script>
</body>
</html>